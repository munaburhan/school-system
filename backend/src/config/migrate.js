...
const createTables = async () => {
    ...
    `);
        ...
        await client.query(`ALTER TABLE staff ADD COLUMN IF NOT EXISTS staff_id VARCHAR(50)`);
        await client.query(`ALTER TABLE staff ADD COLUMN IF NOT EXISTS joining_date DATE`);
        await client.query(`ALTER TABLE staff ADD COLUMN IF NOT EXISTS staff_category VARCHAR(100)`);

        // Create Unique Constraint safely
        await client.query(`
          DO $$ BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'staff_staff_id_key') THEN
        ...
        await client.query(`
          DO $$ BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'idx_staff_staff_id') THEN
              CREATE INDEX idx_staff_staff_id ON staff(staff_id);
            END IF;
          END $$;
        `);
        
        await client.query('CREATE INDEX IF NOT EXISTS idx_teacher_assignments_teacher ON teacher_assignments(teacher_id)');

        // Drop restrictive role CHECK constraint if it exists
        await client.query(`
          DO $$ BEGIN
